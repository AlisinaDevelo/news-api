...
- **Workflow runtime** — every job has an explicit timeout; replaceable workflows cancel stale same-branch runs through a workflow-scoped concurrency group. CodeQL preserves scheduled scans, and release jobs are intentionally not cancelable. Run `npm run workflow:bounds` before merging workflow changes.

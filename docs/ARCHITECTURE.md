...
6. **Response mapping** keeps legacy `/api/articles*` endpoints backward compatible with raw arrays, while `/api/v1/*` returns `{ data, meta }` envelopes with `requestId`, normalized filters, `X-API-Version`, weak `ETag` validators, and search cache status in both metadata and headers. `If-None-Match` uses weak comparison and matching `GET`/`HEAD` requests return a bodyless `304`; request IDs and cache-state metadata are excluded from the validator input. Provider `429` and `503` failures retain explicit public statuses and may include a normalized `Retry-After` header.

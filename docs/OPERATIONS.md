...
- `HTTP_TIMEOUT_MS` limits each provider attempt; `UPSTREAM_TOTAL_TIMEOUT_MS` caps the entire search, including retries and backoff. Total-budget exhaustion is recorded as `timeout`, counts toward the circuit breaker, and uses the normal `502` upstream error. Client disconnects and process shutdown remain `canceled` and do not count as provider failures; a canceled half-open probe returns the circuit to a fresh timed-open window so later recovery remains possible.
- Provider `429` responses return `429` with code `upstream_rate_limited`; provider `503` responses return `503` with code `upstream_unavailable`. A valid provider `Retry-After` value is normalized to delta-seconds and capped at 86400 seconds, while circuit-open `503` responses provide the bounded remaining cooldown (or one second while a half-open probe is running). Arbitrary upstream headers are never forwarded.
- The circuit breaker counts network failures, `408`, `425`, `429`, and 5xx responses; permanent 4xx responses remain visible as upstream errors without opening the circuit and release any half-open recovery slot.

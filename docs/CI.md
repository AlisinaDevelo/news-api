...
13. **`npm run smoke:docker`** — Compose boots Redis, the production image, a slow GNews-compatible fake provider, and two rate-limited API replicas. It proves shared quotas and cross-replica cold-miss coordination, runs the authenticated HTTP smoke including private cache policy, ETag reuse, and a bodyless `304`, then stops Redis to verify strict replicas become unready while liveness and the cache-only replica remain healthy. Redis restart must restore readiness without restarting the APIs.

...
4. **Observe** — each step emits structured Pino logs (carrying `x-request-id`), Prometheus counters (cache hit/miss/stale fallback/coalescing/cancellation, upstream outcome, transport events and suppressed transport warnings, latency histogram), and optional OpenTelemetry spans.
